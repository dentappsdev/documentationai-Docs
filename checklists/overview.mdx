...
<Callout kind="tip">
If you select an operatory room and your office has multiple operatories, a **scope selector** appears allowing you to create the checklist for **This room only** or **All operatories** at once. Selecting "All operatories" creates a [shared checklist](/checklists/shared-checklists) -- a single schedule linked to all matching rooms with centralized editing and per-room assignment overrides.
</Callout>

## Shared checklists

When your practice has multiple rooms of the same type (e.g., 7 operatories), you can create a **shared checklist** that applies to all of them at once. Shared checklists are managed centrally -- any changes to the frequency, description, or SOP update across all linked rooms -- while still allowing per-room assignment overrides.

In the Room view, shared checklists appear in a dedicated card at the top of the room category group with an emerald badge showing coverage (e.g., "All 7 operatories"). Click the shared task card to open the **Shared Task Detail Sheet**, where you can:

- View each linked room with its current status and assigned staff member
- Change per-room assignees independently using the inline edit button
- Assign staff to specific dates for individual rooms
- Edit the shared schedule (frequency, SOP, evidence requirements) for all rooms at once
- Unlink individual rooms from the shared schedule

<Callout kind="info">
Only users with the **admin** or **manager** role see the scope selector when creating checklists.
</Callout>

See [Shared Checklists](/checklists/shared-checklists) for the full guide on creating, editing, and managing shared checklists.
